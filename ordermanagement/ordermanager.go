...
func Run(
	...
	syncRespCh <-chan SyncResponse, //mottar fra peer ved rejoin
	syncRespOutCh chan<- SyncResponse, //sender til peer ved rejoin
) {
	ws := NewWorldState()
	...
	for {
		...
		case pe := <-peerEventCh:
			...
			if !exists || prev != pe.Alive {
				...
				if pe.Alive && wasDead {
					syncRespOutCh <- SyncResponse{
						FromID:              myID,
						ConfirmedHallOrders: ws.ConfirmedHallOrders,
						ConfirmedCabOrders:  copyAllCabOrders(ws.ConfirmedCabOrders),
					}
...
/* 
			...
			if (peerOrder.Phase > info.Phase) && !(peerOrder.Phase == Confirmed && info.Phase != Unconfirmed) { 
		...
		case resp := <-syncRespCh:
			...
			for floor := 0; floor < config.N_FLOORS; floor++ {
				if resp.ConfirmedHallOrders[floor][config.BT_HallUp] {
					key := makeOrderKey("", floor, config.BT_HallUp)
					if confirmOrderInWorldState(&ws, key) {
						changed = true
					}
				}
				if resp.ConfirmedHallOrders[floor][config.BT_HallDown] {
			...
			for ownerID, floors := range resp.ConfirmedCabOrders {
				for floor, confirmed := range floors {
					if confirmed {
...
func copyAllCabOrders(src map[string][]bool) map[string][]bool {
    dst := make(map[string][]bool)
    for id, floors := range src {
        floorsCopy := make([]bool, len(floors))
        copy(floorsCopy, floors)
        dst[id] = floorsCopy
    }
    return dst
}